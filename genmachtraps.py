...
TYPE_MAP = {
    ...
    'kern_return_t'                        : 'mach2::kern_return::kern_return_t',
    'mach_error_t'                         : 'mach2::error::mach_error_t',
...
with open('src/aarch64/syscall/mach_impl.rs', 'w') as fp:
    ...
    for trap in trap_table:
        if trap.name != 'kern_invalid':
            if trap.ret_ty.is_ptr or any(v.is_ptr for v in trap.fn_sig) or trap.name in IMPLEMENTED_TRAPS:
                if trap.fn_sig:
...
with open('src/aarch64/virtos/syscall/mach/delegate.rs', 'w') as fp:
    ...
    for trap in trap_table:
        ...
        if not trap.ret_ty.is_ptr and all(not v.is_ptr for v in trap.fn_sig) and trap.name not in IMPLEMENTED_TRAPS:
            continue
        ...
        if trap.fn_sig:
            ...
            print(f'pub fn {trap.name}({cpu}: &Cpu, {args}: ARG_{trap.name}) -> {trap.ret_ty.rust_type} {{', file = fp)
        else:
