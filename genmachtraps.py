...
POINTER_TYPES = {
    ...
    'mach2::mach_voucher_types::mach_voucher_attr_raw_recipe_t',
}
...
with open('src/aarch64/syscall/mach_impl.rs', 'w') as fp:
    print('//! Generated by `genmachtraps.py`, DO NOT EDIT.', file = fp)
    print(file = fp)
    print(MACH_IMPL_PRELUDE.strip(), file = fp)

    for trap in trap_table:
        if trap.name != 'kern_invalid':
            if trap.ret_ty.is_ptr or any(v.is_ptr for v in trap.fn_sig):
                if trap.fn_sig:
                    print(f'            {trap.name}(args),', file = fp)
                else:
                    print(f'            {trap.name}(),', file = fp)
            else:
                if trap.fn_sig:
                    print(f'            {trap.name}(..),', file = fp)
                else:
...
with open('src/aarch64/virtos/syscall/mach/delegate.rs', 'w') as fp:
    print('//! Generated by `genmachtraps.py`, DO NOT EDIT.', file = fp)
    print(file = fp)
    print('use mach2::kern_return::{KERN_NOT_SUPPORTED, kern_return_t};', file = fp)
    print(file = fp)
    print('use crate::aarch64::{cpu::Cpu, syscall::mach::*};', file = fp)
    print(file = fp)
    ...
    for trap in trap_table:
        ...
        if not trap.ret_ty.is_ptr and all(not v.is_ptr for v in trap.fn_sig):
            continue

        if trap.fn_sig:
            print('#[inline]', file = fp)
            print(f'pub fn {trap.name}(_cpu: &Cpu, _args: ARG_{trap.name}) -> kern_return_t {{', file = fp)
        else:
            print('#[inline]', file = fp)
            print(f'pub fn {trap.name}(_cpu: &Cpu) -> kern_return_t {{', file = fp)

        if trap.name not in UNSUPPORTED_TRAPS:
            print('    todo!();', file = fp)
        else:
            print('    KERN_NOT_SUPPORTED', file = fp)
...
subprocess.check_call([
    ...
    'src/aarch64/syscall/mach_impl.rs',
    'src/aarch64/virtos/syscall/mach/delegate.rs',
])
